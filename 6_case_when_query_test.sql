...
select avg(balance_201311), avg(balance_201312) from casa_201312
group by cust_id;

select (case when balance_201312 > balance_201311*1.1 
then 1 else 0 end) as accept from casa_201312;

select avg(case when balance_201312 > balance_201311*1.1 
then 1 else 0 end) as 성공률 from casa_201312;

select (sum(case when balance_201312 > balance_201311*1.1 
then 1 else 0 end) /count(*)) as 성공률  from casa_201312;

select (sum(balance_201312) - sum(balance_201311)) as 증가분
from casa_201312;

select (sum(balance_201312)-sum(balance_201311))/sum(balance_201311) 
as 증가율  from casa_201312;
...
select count(*) from stud_score where math_score is not null;
...
coalesce(eng_score,0)+  coalesce(phil_score, 0)+
coalesce(music_score,0)) as total_sum from stud_score
order by total_sum desc limit 1 ;

music_score)) as total_score from stud_score 
select max(math_score), min(math_score) from stud_score;

select avg(math_score), avg(eng_score), avg(phil_score),
avg(music_score) from stud_score;

select student_id, stddev(math_score), stddev(eng_score), 
stddev(phil_score), stddev(music_score) from stud_score;

select student_id, 
stddev(math_score, eng_score, phil_score, music_score) 
from stud_score;

select var_samp(math_score), var_samp(eng_score), 
var_samp(phil_score), var_samp(music_score) from stud_score;
...
3. 연봉인상 기준이 직급과 영어점수에 따라 다음과 같다면 
사원별 인상되는 연봉금액을 계산하시오. 
90점 이상 -> 5%
90점 미만 -> 0%
4. 위와 같이 연봉이 인상된다면 추가되는 임금비용은 얼마인가?*/

select avg(current_sal) from staff_sal group by job;
...
else current_sal end as new_sal
from staff_sal;
...
when job='officer' and eng_score <= 90 then current_sal*1.5
...
sum(case when job ='clerk' and eng_score > 90 then current_sal*1.12 
when job ='clerk' and eng_score <= 90 then current_sal*1.07
when job='officer' and eng_score > 90 then current_sal*1.1
