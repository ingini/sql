...
INSERT ignore INTO CUST_INFO VALUES ('8301111999999', 'JIHUN', 0, 'Kim', 330.08);
INSERT ignore INTO CUST_INFO VALUES ('7012012888888', 'JINYOUNG',1,'LEE', 857.61);
INSERT ignore INTO CUST_INFO VALUES ('6705302777666', 'MIJA',1,'HAN', -76.77);
INSERT ignore INTO CUST_INFO VALUES ('8411011555666', 'YOUNGJUN',0,'HA', 468.54);
INSERT ignore INTO CUST_INFO VALUES ('7710092666777', 'DAYOUNG',1,'SUNG', -890);
INSERT ignore INTO CUST_INFO VALUES ('7911022444555', 'HYEJIN',1,'SEO', 47.44);
...
select (case when sex=0 then 'm' else 'f' end)  from cust_info;
select (case when sex=0 then 'm' else 'f' end)  from cust_info;
select concat (last_nm, ' ', FIRST_NM) from cust_info;
select format(annl_perf, 2) from cust_info;
select round(annl_perf, 2) from cust_info;
...
select mid(name,2, 3) from vendor_info;
...
select staff_nm, floor(datediff(now(), birth_dt)/365) as age
from clerk order by age;
select staff_nm, (year(now())- year(birth_dt)) as age
from clerk order by age;
select staff_nm, (year(now())- year(birth_dt)) as age
from clerk order by age;
select staff_nm, birth_dt, adddate(birth_dt, 30) from clerk;
select gender, avg(round(datediff(now(), birth_dt)/365)) 
as age from clerk group by gender;
select def_nm, avg(round(datediff(now(), birth_dt)/365)) 
as age from clerk group by def_nm order by age limit 1;
